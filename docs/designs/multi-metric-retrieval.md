...
1. 识别用户明确列出的多个已登记指标；
2. 对每个指标分别完成 Dense Retrieval（稠密向量检索）；
3. 确认这些指标可以在同一条 SQL 中安全组合；
4. 合并表、列、关联关系、日期和固定指标过滤条件；
5. 只调用一次 LLM（大语言模型）生成一条 SQL；
6. 在执行前用 Deterministic Guard（确定性校验）确认所有指标均被正确实现；
7. 任一指标缺失、冲突或链路失败时整体失败，不返回部分结果。
...
当前代码已经具备 TABLE、COLUMN、METRIC 三类检索、关系图连接、Prompt 组装、SQL Scope Guard（范围校验）和 SQL Safety Guard（安全校验），但仍是单指标逻辑：

- 指标候选最终只选择一个；
- 指标上下文只输出一个指标对象；
- 技术失败统一允许静态回退；
- SQL Guard 不知道本次必须输出哪些指标；
- 关系路径只判断可达性，没有验证聚合关联是否会放大行数；
- 运行时快照没有完整的指标名称和别名映射。

因此本次不是替换现有检索，而是在现有单指标链路旁增加一个受限、Fail Closed（失败关闭）的多指标分支。
...
QueryRequest
  ...
  → TABLE Retrieval × 1
  → METRIC Retrieval × N
  → Metric Completeness + Compatibility Check
...
### 8.2 Per-Metric Retrieval（逐指标召回）

每个已解析指标独立执行一次 METRIC Dense Retrieval：

- 查询文本使用用户原始指标短语及已确认规范指标名；
- 召回结果必须包含该短语映射出的目标 `document_id`；
- 每个指标单独记录 Top-K 证据；
- 任一目标没有召回即 `NO_METRIC_HIT`，整体失败；
- 不允许用另一个相似指标替换目标指标。

目录映射负责“用户明确说的是谁”，向量召回负责“在线检索链路确实找到了它”。二者必须同时成立。
...
保留现有扁平命中证据，同时增加按“用户指标项”分组的证据，能够回答：
...
接入逐指标召回、表列合并、关系基数校验、结构化 Context 和证据；实现多指标 Fail Closed。
