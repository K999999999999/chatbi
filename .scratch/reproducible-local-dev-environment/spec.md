...
当前仓库已有 Dev Container 配置，但开发数据初始化、运行数据生命周期和本地入口尚未形成可从新 clone 验收的统一闭环。目标同时支持 Dev Container 和 WSL / Linux 本地环境：Dev Container 是推荐标准入口，WSL / Linux 是受支持入口。两者不得依赖 W11 旧项目目录、旧 PostgreSQL / Qdrant 物理数据或某个开发者电脑上的运行资产。

## Solution

新开发者从干净 clone 开始，可以选择推荐的 Dev Container，或受支持的 WSL / Linux 本地环境。两种入口使用同一套 `uv` 依赖安装、`.env` 配置、Docker Compose 基础设施启动和 PostgreSQL 初始化流程。首次启动后得到可用的 PostgreSQL 开发环境及可连接的 Qdrant 服务；PostgreSQL 首次初始化创建 Sales Mart、Control DB、所需角色和权限，并载入完整的确定性合成开发数据；后续日常启动保留数据且不重复播种。

Dev Container 重建只重建开发工作区容器，不重置 PostgreSQL 或 Qdrant 的 named volume。数据库重置和 Qdrant 索引重建 / 清除仍是单独、明确的操作。
...
- 作为新开发者，我可以选择推荐的 Dev Container，或受支持的 WSL / Linux 本地环境；两种入口共用 `uv`、`.env`、Docker Compose 和数据库初始化流程，不需要旧机器数据或手工执行多段 SQL。
- 作为开发者，我可以重建 Dev Container 工作区而保留 PostgreSQL 和 Qdrant named volume 中已有的开发数据与索引。
...
- Dev Container 是推荐标准入口；WSL / Linux 本地环境是受支持入口。两者使用同一套 `uv` 安装、`.env`、Compose 服务、数据库首次初始化和显式索引构建流程；不维护两套初始化脚本或不同的开发数据来源。
- PostgreSQL 与 Qdrant 使用当前 Compose 项目专属 named volume，并与 Dev Container 工作区容器的创建、删除和重建解耦。Rebuild Dev Container 不得删除或重置这两个数据卷。
- 只有明确执行 PostgreSQL 重置或 Qdrant 数据清除操作才会删除对应数据卷。Dev Container / WSL 日常关闭、重启或重建均不触发这些操作。
- 两种入口的 Runbook 均说明 `.env` 的放置、Compose 项目身份、容器 / 宿主机连接方式、初始化与服务验证命令；平台差异只在必要的路径或连接端点配置处说明。
...
- PostgreSQL、Qdrant 和 Dev Container / WSL / Linux 中运行的应用之间的 host、port、database、账号和认证配置要一致且明确。Compose 不再使用 `./data/postgres` 或 `./data/qdrant` 作为运行数据目录。
...
应用在推荐的 Dev Container 和受支持的 WSL / Linux 本地环境中，都由现有 `uv` 应用入口启动；Docker Compose 负责本地基础设施和数据库生命周期。本 Feature 不增加新的查询链路，不改变 Query API、SQL Guard、Online Retrieval、Semantic、Authorization 或生产业务逻辑 Contract。
...
从干净 clone、没有任何 PostgreSQL / Qdrant 运行目录和旧 W11 数据的环境完成以下验收；Dev Container 和 WSL / Linux 本地环境均需覆盖：

1. 两种入口均可按 `.env.example` 准备本地配置，使用同一套 `uv sync`、首次 `docker compose up` 和数据库初始化流程成功建立环境。
...
6. Rebuild Dev Container 后 PostgreSQL 和 Qdrant named volume 仍保留原数据；单独执行对应重置 / 清除命令后，才删除目标数据。
...
- 精确开发 Seed 行数与生成算法、Dev Container / WSL 对 Compose 的连接端点细节、自动数据库初始化的执行编排、首次管理员命令如何与自动迁移拆分、模型仓库 revision、Qdrant 历史集合清理策略和本地临时 PostgreSQL 的启动方式属于 Implementation Design 细节；后续设计应选定并给出两种入口共用的可执行命令与恢复步骤。
...
- Runbook 同时说明推荐的 Dev Container 和支持的 WSL / Linux 本地入口；两者共享依赖、配置、Compose、初始化和重建契约，并明确 Dev Container 重建不删除持久数据卷。
