...
| --- | --- | --- |
| `src/online_query/` Online Query | `OnlineQueryService.query()` 负责查询编排；`retrieval/` 负责 Online Retrieval 的运行时、资源检索、指标字段依赖、候选范围、关系解析、上下文和有限 Trace / 结果辅助；`sql_guard/` 负责 SQL 范围、认证 Join、Multi-Metric 校验和作用域辅助；`query_trace.py` 负责查询链路标记 | `tests/online_query/`；`docs/specs/online-query.md`、`docs/specs/online-retrieval.md`、`docs/designs/online-query.md`、`docs/designs/online-retrieval.md`；`docs/acceptance/online-retrieval-20260907.md` |
...
| --- | ---: | --- | --- |
| `src/online_query/retrieval/retrieval.py` | 34,243 → 23,958 bytes | `OnlineRetriever.retrieve()` 编排 Retrieval、候选闭包和关系解析；候选范围、指标字段依赖、结果构造与 Trace 辅助已移至同目录模块 | Ticket 03；`tests/online_query/test_retrieval*.py` 和完整确定性测试 |
| `src/online_query/retrieval/resource_retrieval.py` | — → 11,422 bytes | 负责 TABLE、COLUMN、METRIC 检索和命中结果转换；指标字段依赖已移至 `metric_requirements.py` | Ticket 03；`tests/online_query/test_retrieval*.py` 和完整确定性测试 |
| `src/online_query/sql_guard/sql_guard.py` | 25,129 → 4,963 bytes | `validate_candidate_scope()`、`validate_sql()`、`validate_multi_metric_sql()` 保留公共入口；作用域、认证 Join 和 Multi-Metric 校验已分别移至同目录模块 | Ticket 04；`tests/online_query/test_sql_guard.py` 和 SQL 安全回归 |
| `src/online_query/service.py` | 19,514 → 24,660 bytes | `OnlineQueryService.query()` 保留 Query Understanding、Retrieval、Prompt、LLM、SQL Guard 和数据库主编排；Trace 辅助已移至 `query_trace.py` | Ticket 05；服务、Query API、请求 ID 和完整确定性测试 |
