...
COLUMN Top-K = 10，threshold = 0.25
METRIC Top-K = 5，threshold = 0.30
```

其中 `COLUMN = 10` 是所有候选表合计最多 10 个列；`METRIC = 5` 是候选数，不是最终输出指标数。V1 一条查询最多明确请求 3 个指标。
...
- METRIC 在存在指标意图时执行一次综合检索，最多保留 5 个候选，低于 `0.30` 的结果不进入有效候选集。
- METRIC 路线不按每个指标分别发起检索。
- 无指标意图的实体类查询跳过 METRIC，空的指标上下文不视为技术失败。
- COLUMN 检索必须在 TABLE 候选范围内进行，最多保留所有候选表合计 10 个列，低于 `0.25` 的结果不进入有效候选集。
- 不增加字段白名单、黑名单、加权或条件匹配规则引擎。
- 不通过额外的分组字段解析、全库字段扫描或隐藏补充机制绕过候选范围。
- 不使用中间表补入机制扩大候选表范围。

### 4. Metric（指标）

- V1 一条查询最多明确请求 3 个指标。
- 指标候选数为 5，实际输出指标数为用户明确请求的 1～3 个指标。
...
- 指标公式引用的物理字段不自动补入 COLUMN 候选；如果必需字段没有被 COLUMN 有效命中，返回 `CANNOT_ANSWER`。
...
- Join Key 不占用 COLUMN Top-10；它作为 Graph 的结构事实提供给结构化上下文和 SQL Guard。
...
- TABLE Top-5、COLUMN 总 Top-10、METRIC Top-5 和三个阈值；
- 实体类查询跳过 METRIC；
- 单指标和多指标使用同一编排流程；
- 多指标最多 3 个、全部命中、全部返回且顺序正确；
- 多指标来源、时间口径、固定条件或用户范围不兼容时拒答；
- 指标公式、固定过滤条件、`time_field` 和 `data_source` 不被修改；
- 指标必需字段未被 COLUMN 命中时拒答，不自动补充；
- 候选表范围内的 COLUMN 检索；
...
- 不自动补充未被 COLUMN 命中的公式字段、时间字段或业务字段。
...
- 课程示例中出现过表 Top-3、字段 Top-10 / Top-12、指标 Top-3 和依赖展开；ChatBI V1 经本轮确认采用 TABLE=5、COLUMN=10、METRIC=5，并关闭依赖展开。
