...
- 调用失败、超时、空响应、非法 JSON 或响应转换失败映射为 `LLM_ERROR`；
...
已完成独立 `QueryUnderstandingAdapter`、专用 Prompt、JSON Contract 转换、`LLM_ERROR` 映射和 `llm.query_understanding` Trace 阶段；定向测试及 `tests/online_query` 全量测试通过。
